...
jQuery(function () {
  ...
  if (dataTableLevels.length) {
    ...
    dt_level = dataTableLevels.DataTable({
      ...
      pageLength: 100,
      ajax: {
        url: baseUrl + 'fleet-category/list',
      ...
      columnDefs: [
        ...
        },
        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            var categoryId = full['category_id'];
            ...
            var button = `<button class="btn btn-sm btn-icon btnUpdate" data-object='${JSON.stringify(full).replace(/'/g, "&#39;")}'><i class="ti ti-edit"></i></button>
                          <button class="btn btn-sm btn-icon btnDelete" data-id="${full['category_id']}"><i class="ti ti-trash"></i></button>`;
...
function buildCategoryTreeList(categories, parentId = null, level = 0) {
  ...
  for (let category of categories) {
    if (category.parent_id == parentId || (parentId == null && category.parent_id == 0)) {
      ...
      result = result.concat(buildCategoryTreeList(categories, category.category_id, level + 1));
    }
  }
...
function categoryTreeOptions(categoryTree) {
  ...
  categoryTree.forEach((category, key) => {
    if(category.parent_id == 0){
      optionsHtml += `<option value="${category.category_id}" class="level_${category.level}">${''.repeat(category.level)}${category.category_name}</option>`;
    }
