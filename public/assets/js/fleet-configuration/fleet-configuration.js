        ...
        response.data.forEach((e, i) => {
          tableBody += `
                    <tr>
                        ...
                        <td class="align-top">${e['configuration_type']}</td>
                        <td class="align-top">${e['configuration_name']}</td>
                        <td class="align-top text-start">${e['configuration_setting']}</td>
                        <td class="align-top text-center">
                          <img src="/uploads/configuration/${e['configuration_photo']}" width="100px" height="100px">
                        </td>
                        <td class="align-top text-nowrap" style="white-space: nowrap;">
                          <button class="btn btn-sm btn-icon btnUpdate" data-object='${JSON.stringify(e).replace(/'/g, "&#39;")}'><i class="ti ti-edit"></i></button>
                          <button class="btn btn-sm btn-icon btnDelete" data-id="${e['configuration_id']}"><i class="ti ti-trash"></i></button>
                        </td>
                    </tr>`;
        });
...
function deleteItem(id) {
  ...
  $.ajax({
    url: '/fleet-configuration/delete',
    type: 'POST',
    headers: {
      'X-CSRF-TOKEN': csrfToken,
    },
